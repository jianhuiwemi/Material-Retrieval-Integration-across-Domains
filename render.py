...
for i in range(45, 155):
    folder_name = f"000-{i:03d}"  # 格式化为三位数，例如 001, 002, ..., 130
    path = f'/mnt/large_storage/wry/wjh/obj_files/hf-objaverse-v1/{folder_name}'
    model_dirs.append(path)

output_dir = r'/mnt/large_storage/wry/wjh/output2'
root_dir = r'/mnt/large_storage/wry/wjh/figure'
hdri_dir = r'/mnt/large_storage/wry/wjh/hdri_files'
...
def add_uv_mapping(obj):
    if obj.type == 'MESH':
        ...
        print(f"对象 {obj.name} 不是网格对象，无法进行UV映射")

def detect_pbr_textures(folder_path):
    """
    动态检测并返回材质文件夹中的 PBR 贴图字典。
    """
    texture_dict = {}
    ...
    for file_name in os.listdir(folder_path):
        ...
        elif "_Metalness" in file_name:  # 确保Metalness贴图被检测到
            texture_dict['Metalness'] = os.path.join(folder_path, file_name)
            print(f"检测到Metalness贴图: {file_name}")
    
    # 检查是否成功检测到了Metalness贴图
    if 'Metalness' not in texture_dict:
        print(f"未检测到Metalness贴图，文件夹: {folder_path}")
...
def normalize_scene():
    ...
    print(f"规范化后对象的位置和缩放: {[obj.matrix_world for obj in selected_objects]}")
...
def setup_cameras() -> Dict[str, bpy.types.Object]:
    def create_camera(name: str, location: Tuple[float, float, float], scale: float) -> bpy.types.Object:
        ...
        sensor_width_in_mm = 36  # 假设标准35mm相机传感器宽度
        resolution_x_in_px = 1920  # 假设1920像素宽度
        resolution_y_in_px = 1080  # 假设1080像素高度
        ...
        constraint.target.location = Vector((0, 0, 0))  # 目标是中心点
    ...
    nRows = 9
    nCols = 14
    latitude_angles = [5, 15, 25, 35, 45, 55, 65, 75, 95]  # 限制在上半球，纬度角从5度到75度
    radius = 3

    camera_positions = []

    for j in range(nRows):
        latitude = latitude_angles[j] / 180.0 * math.pi  # 将角度转换为弧度
        for i in range(nCols):
            longitude = (i * 360.0 / nCols - 180.0) / 180.0 * math.pi  # 经度角范围保持不变
...
def _create_light(
    ...
    light_data.use_shadow = use_shadow  # 关闭阴影
...
def setup_lighting() -> Dict[str, bpy.types.Object]:
    ...
    key_light = _create_light(
        name="Key_Light",
        light_type="SUN",
        location=(6, 6, 10),  # 提高光源高度
        rotation=(math.radians(45), 0, math.radians(45)),
        energy=5,
        use_shadow=False,  # 关闭阴影
    )
...
def set_render_settings():
    ...
    # 设置颜色管理为标准模式（非 Filmic）
    scene.view_settings.view_transform = 'Standard'  # 或使用 'sRGB'
    ...
    scene.render.image_settings.color_mode = 'RGBA'  # 确保渲染输出带有 Alpha 通道
...
def process_model(model_path, output_dir, pbr_textures_list, hdri_files, start_index, log_file, used_textures, used_hdri, record_file):
    ...
    if not selected_objects:
        print(f"无法导入模型 {model_path} 或模型为空。")
        return start_index

    bpy.ops.object.select_all(action='DESELECT')
    for obj in selected_objects:
        obj.select_set(True)
    print(f"导入并选择的对象: {[obj.name for obj in selected_objects]}")
    ...
    for obj in selected_objects:
        obj.select_set(True)
    print(f"删除骨骼和动画数据后选择的对象: {[obj.name for obj in selected_objects]}")

    try:
        selected_objects = normalize_scene()
    except RuntimeError as e:
        print(f"模型 {model_path} 规范化失败，跳过处理。错误信息: {e}")
        return start_index  # 根据实际情况返回适当的值

    bpy.ops.object.select_all(action='DESELECT')
    for obj in selected_objects:
        obj.select_set(True)
    print(f"应用材质前选择的对象: {[obj.name for obj in selected_objects]}")

    if len(used_textures) == len(pbr_textures_list):
        used_textures.clear()  # 清空材质使用记录
        save_used_records(used_textures, used_hdri, record_file)  # 保存清空后的记录

    available_textures = [t for t in pbr_textures_list if frozenset(t.items()) not in used_textures]
    if available_textures:  # 检查是否有可用材质
        random.shuffle(available_textures)  # 打乱顺序以确保随机性
        pbr_textures = available_textures[0]
    else:
        random.shuffle(pbr_textures_list)
        pbr_textures = pbr_textures_list[0]

    used_textures.add(frozenset(pbr_textures.items()))  # 标记该材质为已使用
    save_used_records(used_textures, used_hdri, record_file)  # 保存使用记录

    print(f"选择的PBR材质: {pbr_textures}")
    ...
    for obj in selected_objects:
        if obj.type == 'MESH':
            try:
                add_uv_mapping(obj)
                print(f"UV映射已应用到对象: {obj.name}")
            except RuntimeError as e:
                print(f"跳过对象 {obj.name} 的UV映射，因为出现错误：{e}")
                continue
            obj.data.materials.clear()
            obj.data.materials.append(new_material)
            print(f"材质 {new_material.name} 已应用到对象: {obj.name}")
        else:
            print(f"对象 {obj.name} 不是网格对象，跳过材质应用。")
...
used_textures, used_hdri = load_used_records(record_file)  # 加载使用记录
...
for model_dir in model_dirs:
    ...
    for idx, model_file in enumerate(model_files, start=start_index):
        ...
        try:
            ...
            save_used_records(used_textures, used_hdri, record_file)  # 在处理每个模型后保存使用记录
        except Exception as e:
            print(f"处理模型 {model_path} 时遇到错误: {e}")
    
    # 在切换到下一个 model_dir 之前，调用重排序和日志更新函数
    reorder_folders_and_update_log(output_dir, log_file)
    print(f"文件夹序号和日志记录在 {model_dir} 处理后重排完成。")

    # 更新 start_index 为下一个 model_dir 的起始编号
